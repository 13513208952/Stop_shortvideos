...
> 一款基于 TensorFlow Lite 实现的 Android 短视频自动拦截工具，伪装为系统组件以尽可能排除用户自控力影响。（不会影响正常卸载）
...
- 目前模型针对 **哔哩哔哩、微信视频号、抖音** 短视频界面进行了有限次训练，对其他平台的识别效果可能有限
- 本项目仅供学习和个人使用，所有代码均由AI生成，请合理且谨慎的使用。
- 当发生问题的时候，建议必要时卸载此应用，谷歌官方的android system webview包名与此软件包名存在差异，请卸载com.android.webview为包名的软件
- 此软件不属于系统应用，且图标与标准谷歌 android system webview存在差异，为了保障隐藏效果，建议手动加入系统自带的隐藏应用列表中。
